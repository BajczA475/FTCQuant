...
summary.df = data.frame(Duration = Definition.key$Duration, Min_Offset = Definition.key$Min_Offset, 
...
##One question we had was which parameter is the most/least "sensitive?" In other words, as we vary each parameter, does the R2 of the various models produced vary a lot or a little? These graphs plot the R squared values from every model (y axis) against the value of each parameter (x axis). It also shows the mean R2 value for all the models from each parameter value in purple. If you run these three graphs, you will see that the R2 varies a lot as the duration parameter is varied but almost not at all when the minimum offset parameter is varied.
...
mag.sum = group_by(summary.df, Min_Offset) %>% summarize(meanR2 = mean(R2))
...
ggplot(summary.df, aes(y=R2, x=Min_Offset)) + geom_point(alpha=0.05, color="black", size=2) + theme_classic()+ geom_point(data=mag.sum, aes(y=meanR2), shape = 23, size=3, color="purple", fill="purple") + geom_line(data=mag.sum, aes(y=meanR2), color="purple", size=1) + scale_x_continuous("Minimum offset (degrees C)") + scale_y_continuous("R squared")
...
ggplot(summary.df, aes(x=Temp_Threshold, y = Duration, fill=R2)) + geom_raster(interpolate = T) + facet_wrap(~Min_Offset) + scale_fill_gradient2(low="red", mid="black", high="green", midpoint = 0.25) 
