#' Freeze-thaw cycle parameter sweeping analysis
#'
...
#' @usage freeze_thaw_analysis(data.list, mag.vec = 0, dur.vec = 1, thres.vec = 0)
...
#' @param mag.vec,dur.vec,thres.vec Vectors of minimum offsets, durations, and temperature thresholds to pass along to the freeze.thaw.sweep function. See the documentation of that function for more details. Each defaults to an uninformed value of 0 (mag.vec, thres.vec) or 1 (dur.vec).
...
#Critical for loop for cycling through the plot data--it runs the parameter sweep we want to run and then stores the results in a Master data sheet.
  ...
  array1 = freeze.thaw.sweep(time.vec = data$date, durations=dur.vec, lower.offsets = mag.vec, thresholds = thres.vec, temp.vec = data[,column])
  ...
  print(paste("Array", column, "is done", sep=" "))
  }
...
Definition.key = melt(array1, varnames = c("Duration", "Min_Offset", "Temp_Threshold"))[,-4]
